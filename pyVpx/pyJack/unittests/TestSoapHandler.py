...
class TestSoapHandler(unittest.TestCase):
   ...
   ## Test HandleRequest FutureVersion. Should suceed (use default version
   #  handler)
   #
   def test_HandleRequest_FutureVersion(self):
      ...
      self.assertTrue(isinstance(obj, Vmodl.Reflect.DynamicTypeManager.MoInstance.Array))
   ...
   def test_Performance(self):
      ...
      p.strip_dirs().sort_stats('time', 'calls', 'cumulative').print_stats(.3)
   ...
   def test_SoapSerializer_SerializeServerFault(self):
      ...
      fault.faultMessage = [Vmodl.LocalizableMessage(key="vim.fault.ProfileUpdateFailed.UpdateFailure")]
