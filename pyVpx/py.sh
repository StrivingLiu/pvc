...
Usage() {
    ...
    if [ "-n" "$IS_VISOR" ]; then
        echo " a) PYTHON b) which python c) toolchain"
    else
        echo " a) PYTHON b) toolchain c) which python"
    fi
    echo "Debug mode: "
    echo " DEBUGMODE=1"
    exit 0
...
export VMWARE_CMD_DIR=$TREELOC/vim/py/vmware-cmd
...
if [ "-n" "$IS_MAC" ]; then
TC_PYTHON=$TCROOT/mac32/python-2.7.8-openssl1.0.1p/bin/python
else
TC_PYTHON=$TCROOT/lin$ARCH/python-2.7.8/bin/python
fi
