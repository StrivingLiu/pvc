   ...
   def __init__(self, **kwargs):
      for property in self._GetPropertyList():
         ...
         if issubclass(ptype, list):
            ...
            value = ""
         elif ptype is long or \
            issubclass(ptype, int) or \
            issubclass(ptype, float):
   ...
   with _lazyLock:
      ...
      if _allowGetSet == True:
         for property in propList:
            ...
            tGlobals = {}
            exec('def Get%s(self): return getattr(self, "%s")' % (suffix, name)
                  + '\n' +
                 'def Set%s(self, v): setattr(self, "%s", v)' % (suffix, name),
                 tGlobals, dic)
